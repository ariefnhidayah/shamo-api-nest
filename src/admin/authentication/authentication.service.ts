...
export class AuthenticationService {
  ...
  async login(loginDto: LoginDto): Promise<ResponseApi> {
    ...
    const user: User = await this.userRepository.findOne({ where: { email: loginDto.email, role_id: 1 } })
    if (!user) {
      throw new HttpException(null, HttpStatus.BAD_REQUEST, { cause: new Error('Un-registered!') })
    }
