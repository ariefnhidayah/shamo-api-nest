...
export class UsersService {
  ...
  async login(loginDto: LoginDto): Promise<ResponseApi> {
    ...
    const user: User = await this.userRepository.findOne({
      where: { email: loginDto.email, role_id: 2 },
    });
    if (!user) {
      throw new HttpException(null, HttpStatus.BAD_REQUEST, {
        cause: new Error('Un-registered email!'),
      });
    }
  ...
  _getRandomColor(): string {
    const colors = ['08ab02', '00a1e1', 'ab0202', '7f0084', '926900', 'a70050'];
  ...
  async register(registerDto: RegisterDto): Promise<ResponseApi> {
    ...
    if (user) {
      throw new HttpException(null, HttpStatus.BAD_REQUEST, {
        cause: new Error('E-mail has been used!'),
      });
    }
